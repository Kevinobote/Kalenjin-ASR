...
\title{Low-Resource ASR for Kalenjin: Transfer Learning and Language Model Integration

}

\author{\IEEEauthorblockN{1\textsuperscript{st} Kevin Omondi Obote}
...
\IEEEauthorblockA{\textit{Department of Languages and Lingustics} \\
...
\begin{enumerate}
    \item A modular preprocessing pipeline with audio quality assessment (SNR, spectral features, VAD-based silence trimming) and Kalenjin-specific text normalization preserving the phonemically significant \textit{ng'} marker.
...
\begin{itemize}
    ...
    \item Experimental benchmarks that evaluate CTC-based architectures in severe resource-constrained environments \cite{b25}.
\end{itemize}
...
\fbox{\parbox{0.88\columnwidth}{\scriptsize
\centering
\textbf{Raw Common Voice Data (MP3 + TSV)}\\
$\downarrow$\\
\textbf{Audio}: Resample 16\,kHz $\rightarrow$ VAD $\rightarrow$ Trim $\rightarrow$ Normalize\\
$\downarrow$\\
\textbf{Text}: Lowercase $\rightarrow$ Apostrophe Std. $\rightarrow$\\
Orthographic Map $\rightarrow$ Filter $\rightarrow$ CTC Delimiters\\
$\downarrow$\\
\textbf{Quality}: SNR${\geq}$10\,dB, 0.5${\leq}$dur${\leq}$20\,s, ZCR${\leq}$0.3\\
$\downarrow$\\
\textbf{HuggingFace DatasetDict} (train / val / test)\\
$\downarrow$\\
\textbf{Wav2Vec2-XLS-R-300M}: Stage\,1 (frozen) $\rightarrow$ Stage\,2 (unfrozen)\\
$\downarrow$\\
\textbf{KenLM 5-gram + Beam Search Decoding}
}}
\caption{End-to-end ASR pipeline architecture.}
\label{fig:pipeline}
\end{figure}
...
\begin{figure}[htbp]
\centerline{\includegraphics[width=\columnwidth]{figures/split_dist.png}}
...
\begin{figure}[htbp]
\centerline{\includegraphics[width=0.85\columnwidth]{figures/char_freq.png}}
...
\begin{figure}[htbp]
\centerline{\includegraphics[width=0.85\columnwidth]{figures/word_freq.png}}
...
\begin{figure}[htbp]
\centerline{\includegraphics[width=0.85\columnwidth]{figures/zipf.png}}
...
\end{equation}
where $L_{\text{trimmed}}$ and $L_{\text{original}}$ denote the trimmed and original signal lengths, respectively. The effectiveness of silence trimming is quantified by the speech ratio $\rho$ (\eqref{eq:speech_ratio}). Fig.~\ref{fig:speech_ratio} shows the distribution across splits, with means ranging from 0.639 (train) to 0.693 (test), indicating that 31--36\% of original audio consisted of non-speech segments.

\begin{figure}[htbp]
\centerline{\includegraphics[width=0.85\columnwidth]{figures/speech_ratio.png}}
...
Fig.~\ref{fig:audio_quality} presents the distribution of key audio quality metrics across splits, and Fig.~\ref{fig:duration} shows the utterance duration distribution. Fig.~\ref{fig:sc_vs_dur} reveals the relationship between spectral centroid and utterance duration, showing that shorter utterances tend to exhibit higher spectral centroids, likely due to the proportionally greater influence of consonant energy in brief utterances.

\begin{figure}[htbp]
\centerline{\includegraphics[width=\columnwidth]{figures/audio_quality_boxplot.png}}
\caption{Audio quality metrics by split. SNR values are consistently high ($>$25\,dB), spectral centroids fall within the expected speech range (200--8000\,Hz), and zero-crossing rates indicate clean recordings.}
\label{fig:audio_quality}
\end{figure}

\begin{figure}[htbp]
\centerline{\includegraphics[width=0.85\columnwidth]{figures/duration_dist.png}}
...
\begin{figure}[htbp]
\centerline{\includegraphics[width=0.85\columnwidth]{figures/sc_vs_dur.png}}
...
Table~\ref{tab:textnorm} presents representative normalization examples from the corpus.
...
\begin{figure}[htbp]
\centerline{\includegraphics[width=0.85\columnwidth]{figures/wordcount_dist.png}}
...
\hline
Spect. centroid & 200--8000\,Hz & Validate speech content \\
\hline
...
\begin{figure}[htbp]
\centerline{\includegraphics[width=0.85\columnwidth]{figures/snr_dist.png}}
...
The foundation of this ASR system is Wav2Vec2-XLS-R-300M \cite{b15}, a cross-lingual speech representation model pre-trained on 436,000 hours of unlabeled speech spanning 128 languages.
...
The latent representations are projected to dimension $D = 1024$ and processed by a 24-layer Transformer encoder with 16 attention heads. Each Transformer layer applies multi-head self-attention followed by a position-wise feed-forward network:
\begin{equation}
\text{Attention}(\mathbf{Q}, \mathbf{K}, \mathbf{V}) = \text{softmax}\left(\frac{\mathbf{Q}\mathbf{K}^\top}{\sqrt{d_k}}\right)\mathbf{V}
\label{eq:attention}
\end{equation}
where $\mathbf{Q}, \mathbf{K}, \mathbf{V} \in \mathbb{R}^{L \times d_k}$ are the query, key, and value matrices, and $d_k = D/16 = 64$ is the dimension per head. The output is a sequence of contextualized representations $\mathbf{H} = [\mathbf{h}_1, \ldots, \mathbf{h}_L] \in \mathbb{R}^{L \times D}$.

\subsubsection{CTC Head}
A linear projection maps the Transformer outputs to the vocabulary space:
\begin{equation}
\mathbf{y}_t = \mathbf{W}_{\text{ctc}} \mathbf{h}_t + \mathbf{b}_{\text{ctc}}
\label{eq:ctc_head}
\end{equation}
where $\mathbf{W}_{\text{ctc}} \in \mathbb{R}^{|\mathcal{V}| \times D}$ and $\mathbf{b}_{\text{ctc}} \in \mathbb{R}^{|\mathcal{V}|}$, with $|\mathcal{V}| = 32$. The model totals 315.5 million parameters.

\subsubsection{CTC Loss}
The Connectionist Temporal Classification loss \cite{b12} marginalizes over all valid alignments $\boldsymbol{\pi}$ between the input sequence and target label sequence $\mathbf{l}$:
\begin{equation}
\mathcal{L}_{\text{CTC}} = -\ln \sum_{\boldsymbol{\pi} \in \mathcal{B}^{-1}(\mathbf{l})} \prod_{t=1}^{L} p(\pi_t | \mathbf{x})
\label{eq:ctc_loss}
\end{equation}
where $\mathcal{B}^{-1}(\mathbf{l})$ is the set of all CTC paths that reduce to $\mathbf{l}$ after removing blanks and repeated characters, and $p(\pi_t | \mathbf{x}) = \text{softmax}(\mathbf{y}_t)_{\pi_t}$ is the posterior probability of token $\pi_t$ at time step $t$. The CTC loss was configured with mean reduction and the \texttt{ctc\_zero\_infinity} flag enabled to handle numerical edge cases.
...
Table~\ref{tab:results} presents the complete performance progression. Fig.~\ref{fig:progression} visualizes the WER improvements.
...
\hline
\textbf{Configuration} & \textbf{Data} & \textbf{Epochs} & \textbf{WER} & \textbf{CER} \\
\hline
Baseline CPU & 10\% & 3 & 100.0\% & 100.0\% \\
\hline
GPU Baseline (frozen) & 100\% & 30 & 72.11\% & 44.94\% \\
\hline
Stage 1 Optimized & 100\% & 30 & 69.03\% & --- \\
\hline
Stage 2 Refined & 100\% & 15 & 69.13\% & 20.11\% \\
\hline
Stage 2 + KenLM & 100\% & --- & \textbf{61.75\%} & --- \\
\hline
\end{tabular}
\end{table}

\begin{figure}[htbp]
\centerline{\includegraphics[width=0.85\columnwidth]{figures/performance_progression.png}}
\caption{WER progression across configurations. Data scaling (10\%$\rightarrow$100\%) eliminated degenerate predictions; two-stage training and KenLM integration yielded cumulative 38.25 percentage point reduction.}
\label{fig:progression}
\end{figure}

Key findings:
\begin{itemize}
    \item \textbf{Data scaling}: Increasing from 10\% to 100\% of training data eliminated the blank-prediction pathology, reducing WER from 100\% to 72.11\%.
    \item \textbf{Acoustic adaptation}: Unfreezing the feature encoder in Stage~2 reduced CER from 44.94\% to 20.11\% (55.3\% relative reduction), confirming that language-specific acoustic adaptation is critical.
    \item \textbf{Language modeling}: KenLM integration reduced WER from 69.13\% to 61.75\% (10.7\% relative), correcting word boundary and lexical errors.
    \item \textbf{Real-time performance}: RTF of 0.0091 (110$\times$ faster than real-time) on GPU.
\end{itemize}

\section{Discussion}

The final WER of 61.75\% and CER of 20.11\% fall within expected ranges for extremely low-resource ASR ($<$20 hours), comparable to reported results for similar settings: high-resource languages achieve 3--8\% WER with 10,000+ hours, medium-resource African languages (Swahili, Amharic) achieve 15--30\% with 100--1,000 hours \cite{b40}, and low-resource languages typically achieve 60--80\% with $<$20 hours \cite{b6}.

The CER of 20.11\% indicates strong phonetic accuracy---the model correctly predicts approximately 80\% of characters. The higher WER reflects the compounding effect of character-level errors at the word level and challenges in word boundary segmentation for an agglutinative language. The distribution mismatch between splits (test mean duration 3.89s vs. train 2.84s; test SNR 55.5~dB vs. train 50.7~dB) may also contribute to evaluation variance.
...
We presented a complete ASR framework for Kalenjin, establishing the first documented baseline for this under-resourced Nilotic language. Our three-component approach---modular preprocessing with Kalenjin-specific normalization, two-stage fine-tuning of Wav2Vec2-XLS-R-300M, and KenLM language model integration---achieved a final WER of 61.75\% and CER of 20.11\% from 20.22 hours of training data. The 55.3\% relative CER reduction from feature encoder unfreezing and 10.7\% relative WER improvement from language modeling validate staged acoustic adaptation and lightweight LM integration as effective strategies for low-resource African language ASR.

Future work includes: (1) expanding the language model with external Kalenjin text; (2) exploring alternative foundation models (Whisper, HuBERT); (3) dialect-specific adaptation across Kalenjin sub-varieties; and (4) evaluation on spontaneous speech.

\begin{thebibliography}{00}
\bibitem{b1} A. Baevski, Y. Zhou, A. Mohamed, and M. Auli, ``wav2vec 2.0: A framework for self-supervised learning of speech representations,'' in \textit{Advances in Neural Information Processing Systems}, vol. 33, 2020, pp. 12449--12460.
...
\bibitem{b10} L. R. Rabiner, ``A tutorial on hidden Markov models and selected applications in speech recognition,'' \textit{Proc. IEEE}, vol. 77, no. 2, pp. 257--286, 1989.

\bibitem{b11} G. Hinton \textit{et al.}, ``Deep neural networks for acoustic modeling in speech recognition: The shared views of four research groups,'' \textit{IEEE Signal Process. Mag.}, vol. 29, no. 6, pp. 82--97, 2012.

\bibitem{b12} A. Graves, S. Fern\'andez, F. Gomez, and J. Schmidhuber, ``Connectionist temporal classification: Labelling unsegmented sequence data with recurrent neural networks,'' in \textit{Proc. 23rd Int. Conf. Machine Learning}, 2006, pp. 369--376.
...
\bibitem{b16} W.-N. Hsu, B. Bolte, Y.-H. H. Tsai, K. Lakhotia, R. Salakhutdinov, and A. Mohamed, ``HuBERT: Self-supervised speech representation learning by masked prediction of hidden units,'' \textit{IEEE/ACM Trans. Audio, Speech, Language Process.}, vol. 29, pp. 3451--3460, 2021.
...
\bibitem{b19} D. S. Park, W. Chan, Y. Zhang, C.-C. Chiu, B. Zoph, E. D. Cubuk, and Q. V. Le, ``SpecAugment: A simple data augmentation method for automatic speech recognition,'' in \textit{Interspeech 2019}, 2019, pp. 2613--2617.
